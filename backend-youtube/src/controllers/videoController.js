...
const updateVideo = async (req, res) => {
  try {
    ...
    // uodate cái gì: dữ liệu update video
    ...
    res.status(200).json(`Update video id ${videoId} thành công`);
  } catch (error) {
    console.log(error);
...
const deleteVideo = async (req, res) => {
  try {
    ...
    console.log("dữ liệu sau khi xóa", { result });
    res.status(200).json(`Xóa video id ${videoId} thành công`);
  } catch (error) {
    console.log(error);
    return res.status(500).json("Error ${error}");
  }
