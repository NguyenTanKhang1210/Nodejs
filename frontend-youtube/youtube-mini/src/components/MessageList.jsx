...
const MessageList = ({ onSelectChat, onClose }) => {
  ...
  const messages = [
    {
      ...
      lastMessage: "Chào Phương, mình là Sơn Tùng MTP",
      time: "3 giờ",
    },
    {
      id: 2,
  ...
  useEffect(() => {
    // get user info từ phía local storage
    const token = localStorage.getItem("USER_LOGIN");

    // đecode token (cheat decode)
    ...
    // gửi infor user lên server socketIO để thông báo là user đã online
    ...
    socket.on("onlineUsers", (users) => {
      ...
      // filter user hiện tại ra khỏi danh sách
